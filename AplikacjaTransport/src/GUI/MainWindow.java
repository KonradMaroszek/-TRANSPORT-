...
public class MainWindow {
	...
	ArrayList<Map<String, Object>> contractor_records;
	private int building_idx;
	private int contractor_idx;
	...
	private JTextField kontrahent_imie;
	private JTextField kontrahent_nazwisko;
	private JTextField kontrahent_firma;
	...
	private JTextField kontrahent_nip;
	private JTextField kontrahent_kraj;
	private JTextField kontrahent_wojewodztwo;
	private JTextField kontrahent_miasto;
	private JTextField kontrahent_kod_pocztowy;
	private JTextField kontrahent_nr_lokalu;
	private JTextField kontrahent_ulica;
...
//				String typ_umowy = EmployessPersonalDataChoiceConctractOfEmployment.getSelectedItem().toString();
		...
		JButton btnDodajNaprawe = new JButton("Dodaj naprawe");
		btnDodajNaprawe.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				DefaultTableModel model = (DefaultTableModel) naprawy_tab.getModel();
				String czas_naprawy = model.getValueAt(model.getRowCount() -1, 0).toString();
				String koszt = model.getValueAt(model.getRowCount() -1, 1).toString();
				String naprawione_czesci = model.getValueAt(model.getRowCount() -1, 2).toString();
				String nr_rej = VehiclesInformationTextRegistrationNumber.getText();
				String marka = VehiclesInformationTextBrand.getText();
				String car_model = VehiclesInformationTextModel.getText();
				
				String get_pojazd_id = "Select pojazdy.id_pojazdu from pojazdy join karty_techniczne_pojazdow on pojazdy.id_pojazdu = karty_techniczne_pojazdow.id_pojazdu join dowody_rejestracyjne on pojazdy.id_pojazdu = dowody_rejestracyjne.id_pojazdu ";
				get_pojazd_id += "where marka = '"+marka+"' and model = '"+car_model+"' and nr_rejestracyjny = '"+nr_rej+"'";
				String pojazd_id = "";
				try {
					pojazd_id = executeCommandAndWaitForOutput(get_pojazd_id).get().get(0).get("id_pojazdu").toString();
				} catch (InterruptedException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				} catch (ExecutionException e) {
				...
				String insert_naprawa = "Insert into naprawy (id_pojazdu, naprawione_czesci, koszt, czas_naprawy, id_naprawy) values ";
				insert_naprawa += "("+pojazd_id+", '"+naprawione_czesci+"', "+koszt+", "+czas_naprawy+", naprawa_id_seq.nextval)";
				insert_to_db(insert_naprawa, "", "");
				System.out.println(insert_naprawa);
				model.addRow(new Object[] {null,null,null});
			}
		});
		btnDodajNaprawe.setBounds(500, 40, 121, 33);
		panel_6.add(btnDodajNaprawe);
		...
		VehicleRoutes_tab.setModel(new DefaultTableModel(
			...
			new String[] {
				"Czas wyjazdu", "Miejsce wyjazdu", "Czas trwania trasy"
			}
		));
...
//				"Czas dostarczenia", "Towary", "Adres dostawy", "Odbiorca"
		...
		kontrahent_imie = new JTextField();
		kontrahent_imie.setColumns(10);
		kontrahent_imie.setBounds(126, 102, 200, 20);
		ContractorsPanel.add(kontrahent_imie);
		
		kontrahent_nazwisko = new JTextField();
		kontrahent_nazwisko.setColumns(10);
		kontrahent_nazwisko.setBounds(126, 133, 200, 20);
		ContractorsPanel.add(kontrahent_nazwisko);
		
		kontrahent_firma = new JTextField();
		kontrahent_firma.setColumns(10);
		kontrahent_firma.setBounds(126, 164, 200, 20);
		ContractorsPanel.add(kontrahent_firma);
		
		JButton btnUsuKontrahenta = new JButton("usu\u0144 kontrahenta");
		btnUsuKontrahenta.setBounds(855, 222, 171, 23);
		btnUsuKontrahenta.addActionListener(new ActionListener(){

			@Override
			public void actionPerformed(ActionEvent arg0) {
				// TODO Auto-generated method stub
				String imie = kontrahent_imie.getText();
				String nazwisko = kontrahent_nazwisko.getText();
				String firma = kontrahent_firma.getText();
				String nip = kontrahent_nip.getText();
				String insert_contractor_cmd = "delete from dostawy where imie_odbiorcy = '"+imie+"' and nazwisko_odbiorcy = '"+nazwisko+"' and nip like '%"+nip+"%' and firma like '%"+firma+"%'";
				insert_to_db(insert_contractor_cmd,"","");
			}
		...
		btnDodajKontrahenta.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
			String imie = kontrahent_imie.getText();
			String nazwisko = kontrahent_nazwisko.getText();
			String firma = kontrahent_firma.getText();
			String nip = kontrahent_nip.getText();
			String ulica = kontrahent_ulica.getText();
			String nr_lokalu = kontrahent_nr_lokalu.getText();
			String kod_pocztowy = kontrahent_kod_pocztowy.getText();
			String miasto = kontrahent_miasto.getText();
			String wojewodztwo = kontrahent_wojewodztwo.getText();
			String kraj = kontrahent_kraj.getText();
			String id_miejsca = "";
			try {
				id_miejsca = get_adr_id().get().get(0).get("nextval").toString();
			} catch (InterruptedException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			} catch (ExecutionException e) {
			...
			String insert_adr = get_insert_adres_cmd(ulica, nr_lokalu, kod_pocztowy, miasto, wojewodztwo, kraj, id_miejsca);
			
			String insert_contractor_cmd = "insert into dostawy (id_dostawy, id_trasy, id_miejsca_docelowego, nr_faktury, imie_odbiorcy, nazwisko_odbiorcy, nip, firma)";
			insert_contractor_cmd += " values (inf_dostawa.nextval, inf_trasa.nextval, "+id_miejsca+", inf_faktura.nextval, '"+imie+"', '"+nazwisko+"', '"+nip+"', '"+firma+"')";
			System.out.println(insert_adr);
			System.out.println(insert_contractor_cmd);
			insert_to_db(insert_adr,"","");
			insert_to_db(insert_contractor_cmd,"","");
		...
		btnWyszukajKontrahenta.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) {
				...
				String firma = kontrahent_firma.getText();
				String nip = kontrahent_nip.getText();
				String cmd = "Select * from dostawy join adresy on dostawy.id_miejsca_docelowego = adresy.id_adresu where imie_odbiorcy = '"+imie+"' or nazwisko_odbiorcy = '"+nazwisko+"' or nip = '"+nip+"' or firma = '"+firma+"'";
				try {
					contractor_records = executeCommandAndWaitForOutput(cmd).get();
					contractor_idx = 0;
					set_contractor_informations(contractor_records.get(contractor_idx));
				} catch (InterruptedException e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				} catch (ExecutionException e1) {
		...
		button_8.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String firma = kontrahent_firma.getText();
				String nip = kontrahent_nip.getText();
				String cmd = "update dostawy set informacja = '"+info+"' where imie_odbiorcy = '"+imie+"' or nazwisko_odbiorcy = '"+nazwisko+"' or nip = '"+nip+"' or firma = '"+firma+"'";
				insert_to_db(cmd, "", "");
			}
		});
		...
		button_9.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String firma = kontrahent_firma.getText();
				String nip = kontrahent_nip.getText();
				String cmd = "update dostawy set informacja = '' where imie_odbiorcy = '"+imie+"' or nazwisko_odbiorcy = '"+nazwisko+"' or nip = '"+nip+"' or firma = '"+firma+"'";
				insert_to_db(cmd, "", "");
				textArea.setText("");
			}
		});
		...
		button_12.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(contractor_idx > 0) contractor_idx -= 1;
				set_contractor_informations(contractor_records.get(contractor_idx));
			}
		});
		...
		button_13.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(contractor_idx < contractor_records.size() - 1) contractor_idx += 1;
				set_contractor_informations(contractor_records.get(contractor_idx));
			}
		});
		ContractorsPanel.add(button_13);
		
		kontrahent_nip = new JTextField();
		kontrahent_nip.setColumns(10);
		kontrahent_nip.setBounds(126, 195, 200, 20);
		ContractorsPanel.add(kontrahent_nip);
		
		JLabel lblPastwo_1 = new JLabel("Pa\u0144stwo:");
		lblPastwo_1.setBounds(424, 260, 110, 20);
		ContractorsPanel.add(lblPastwo_1);
		
		kontrahent_kraj = new JTextField();
		kontrahent_kraj.setColumns(10);
		kontrahent_kraj.setBounds(573, 260, 190, 20);
		ContractorsPanel.add(kontrahent_kraj);
		
		kontrahent_wojewodztwo = new JTextField();
		kontrahent_wojewodztwo.setColumns(10);
		kontrahent_wojewodztwo.setBounds(573, 223, 190, 20);
		ContractorsPanel.add(kontrahent_wojewodztwo);
		...
		kontrahent_miasto = new JTextField();
		kontrahent_miasto.setColumns(10);
		kontrahent_miasto.setBounds(573, 195, 190, 20);
		ContractorsPanel.add(kontrahent_miasto);
		
		kontrahent_kod_pocztowy = new JTextField();
		kontrahent_kod_pocztowy.setColumns(10);
		kontrahent_kod_pocztowy.setBounds(573, 164, 190, 20);
		ContractorsPanel.add(kontrahent_kod_pocztowy);
		...
		kontrahent_nr_lokalu = new JTextField();
		kontrahent_nr_lokalu.setColumns(10);
		kontrahent_nr_lokalu.setBounds(573, 133, 190, 20);
		ContractorsPanel.add(kontrahent_nr_lokalu);
		
		kontrahent_ulica = new JTextField();
		kontrahent_ulica.setColumns(10);
		kontrahent_ulica.setBounds(573, 102, 190, 20);
		ContractorsPanel.add(kontrahent_ulica);
...
//			"Czas dostarczenia", "Towary", "Adres dostawy", "Odbiorca"
	...
	private void set_car_informations(Map<String, Object> car) {
		...
		VehiclesInformationTextMilage.setText(car.get("przebieg_w_km").toString());
		VehiclesMainTextOverview.setText(car.get("nastepny_przeglad").toString().split(" ")[0]);
		current_car_id = car.get("id_pojazdu").toString();
		DefaultTableModel model = new DefaultTableModel();
		model.addColumn("Czas naprawy");
		model.addColumn("Koszt naprawy");
		model.addColumn("Naprawione czesci");
		String get_naprawy_cmd = "Select * from naprawy where id_pojazdu = "+car.get("id_pojazdu").toString();
		String get_routes_cmd = "Select * from przejazdy where id_pojazdu = "+car.get("id_pojazdu").toString();
		DefaultTableModel model_tras = new DefaultTableModel();
		model_tras.addColumn("Czas wyjazdu");
		model_tras.addColumn("Miejsce wyjazdu");
		model_tras.addColumn("Czas trwania trasy");
		try {
			ArrayList<Map<String, Object>> naprawy = executeCommandAndWaitForOutput(get_naprawy_cmd).get();
			for(Map<String, Object> naprawa : naprawy )
			{
				model.addRow(new Object[] {naprawa.get("czas_naprawy").toString(), naprawa.get("koszt").toString(), naprawa.get("naprawione_czesci").toString()});
			}
			
			model.addRow(new Object[] {null,null,null});
			naprawy_tab.setModel(model);
			ArrayList<Map<String, Object>> trasy = executeCommandAndWaitForOutput(get_routes_cmd).get();
			for ( Map<String, Object> trasa:trasy)
			{
				Map<String, Object> miejsce = executeCommandAndWaitForOutput("select * from adresy where id_adresu = "+trasa.get("id_miejsca_startowego").toString()).get().get(0);
				String miejsce_wyjazdu = "" + miejsce.get("miasto").toString() + " "+miejsce.get("ulica").toString() + " " + miejsce.get("nr_lokalu").toString();
				model_tras.addRow(new Object[] {
						trasa.get("data_przejazdu").toString().split(" ")[0], miejsce_wyjazdu, trasa.get("szacowany_czas").toString()
				});
			}
			VehicleRoutes_tab.setModel(model_tras);
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (ExecutionException e) {
	...
	private void set_contractor_informations(Map<String, Object> record) {
		kontrahent_imie.setText(record.get("imie_odbiorcy").toString());
		kontrahent_nazwisko.setText(record.get("nazwisko_odbiorcy").toString());
		kontrahent_firma.setText(record.get("firma").toString());
		kontrahent_nip.setText(record.get("nip").toString());
		kontrahent_ulica.setText(record.get("ulica").toString());
		kontrahent_nr_lokalu.setText(record.get("nr_lokalu").toString());
		kontrahent_kod_pocztowy.setText(record.get("kod_pocztowy").toString());
		kontrahent_miasto.setText(record.get("miasto").toString());
		kontrahent_wojewodztwo.setText(record.get("wojewodztwo").toString());
		kontrahent_kraj.setText(record.get("kraj").toString());
		textArea.setText(record.get("informacja").toString());
	}
