		...
		btnNewButton.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent arg0) 
			{
					...
					String cmd = "select * from dane_osobowe join pracownicy " +
							   "on dane_osobowe.id = pracownicy.id_danych_osobowych where " +
								sql_condition;
					Future<ArrayList<Map<String, Object>>> result = executeCommandAndWaitForOutput(cmd);
					try {
						Map<String, Object> output = result.get().get(0);
			...
			private String getSqlSearchCondition() {
				...
				if(! age.equals(""))
				{
					...
					long wiek = Integer.parseInt(age);

					long diff = now - (wiek * 1000 * 365 * 24 * 60 * 60);
					String today = DateFormat.getDateInstance(DateFormat.SHORT).format(now);
					String searched = DateFormat.getDateInstance(DateFormat.SHORT).format(diff);
					String day = today.split("/")[1];
					String mon = today.split("/")[0];
					String year = searched.split("/")[2];
...
//						EmployessPersonalDataTextWorkplaceAddress.setText(t);
//						EmployessPersonalDataTextStreet
//						EmployessPersonalDataTextLocalNumber
//						EmployessPersonalDataTextPostalCode
//						EmployessPersonalDataTextCity
//						EmployessPersonalDataTextLand
//						EmployessPersonalDataTextCountry
			}
